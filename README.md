...
#### 2.1.1 `acceptable.lst`
**用途**：许可证合规性管理文件
这是一个关键的许可证审核清单，包含所有已批准使用的第三方库许可证的 MD5 哈希值，包含的主要许可证类型包括：GPL (2/3)、LGPL、BSD、MIT、Apache 2.0、PostgreSQL License 等。格式为每行一个条目：
```
<MD5哈希值> <包名/许可证类型>
```
**作用**：
...
#### 2.2.1 `.github/`
`workflows/build-packages.yml` 是 GitHub Actions 工作流配置，目前因为某些原因没有使用起来。
...
#### 2.2.2 `scripts/`
构建系统核心脚本目录，包含所有自动化构建、打包、上传相关的工具脚本，下列是主要脚本说明：
...
##### 2.2.2.11 `pyshebangcheck.sh`
**Python shebang 检查工具**
确保 Python 脚本的 shebang 行（`#!/usr/bin/env python`）在 Windows 环境下正确。

##### 2.2.2.12 `QGISInstallDirDlg.wxs` / `QGISUI_InstallDir.wxs`
**WiX 对话框定义文件**（XML）
...
## 3. 构建流程
参考 [QGISPackagingGuide](https://github.com/xiaobing318/NOTES/blob/main/QGIS-Materials/QGISCompilationNavigation/QGIS-Windows-AMD64/QGISPackagingGuide-Win10/QGISPackagingGuide.md) 文件中所描述的详细步骤。
...
## 5. 许可证管理
项目对许可证合规性有严格要求，构建系统会检测不兼容的许可证组合，如 GPL 与专有许可：
